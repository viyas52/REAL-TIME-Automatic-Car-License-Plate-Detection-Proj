...
def read_license_plate(license_plate_crop):
    ...
    Args:
        license_plate_crop (numpy.ndarray): Cropped image containing the license plate in grayscale.

    Returns:
        tuple: Tuple containing the formatted license plate text and its confidence score, or (None, None) if no text is detected.
    """
    
    # Convert the grayscale image to RGB if needed
    # (uncomment if needed, depending on PaddleOCR requirements)
    # license_plate_crop_rgb = cv2.cvtColor(license_plate_crop, cv2.COLOR_GRAY2RGB)
    
    # Perform OCR on the cropped license plate image
    detections = ocr.ocr(license_plate_crop)

    # Debug: Print detections to understand what is returned
    print("OCR Detections:", detections)

    # Handle the case where detections are None or empty
    if detections is None or len(detections) == 0 or detections[0] is None:
        print("No text detected")
        return None, None

    # Process the detections
    for detection in detections[0]:  # PaddleOCR returns a nested list
        if len(detection) > 1:
            ...
            # Example formats, replace with your actual formats
...
def convert_to_np_array(image):
    """
    Convert an image to numpy array format if it's not already.

    Args:
        image (any): Image to be converted.
